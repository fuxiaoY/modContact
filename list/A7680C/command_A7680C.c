...
extern unsigned char Handle_CMD[NET_MODULE_UART_TXBUF_SIZE];
extern unsigned char Handle_Rec[NET_MODULE_UART_RXBUF_SIZE];

void cmd_BufRecv(tCmd_cb *cb_test)
{
    printf("-------------------------\r\n");
    Modem_Write(cb_test->cmd, cb_test->cmdLen);
    for (uint16 i = 0; i < 5; i++)
    {
        uint32_t single_len_rev = 0;
        single_len_rev= Modem_Read(cb_test->recLen);
        cb_test->recLen +=single_len_rev;
        osDelay(200);
    }
    osDelay(2000);
}

void cmd_BufClear(tCmd_cb *cb_test)
{
    memset(Handle_CMD,0,NET_MODULE_UART_TXBUF_SIZE);
    memset(Handle_Rec,0,NET_MODULE_UART_RXBUF_SIZE);
    memset(cb_test->cmd,0,100);
    memset(cb_test->rec,0,500);
    cb_test->recLen = 0;
    cb_test->cmdLen = 0;
}
//NetworkPara_t networkPara;
#include "api_modem.h"
void NetConfig_Test(void)
{
    osDelay(2000);
    if(false == netConfig()) // 自动配置流程
    {
        printf("Modem: config failed!");
        return;
    }
    //循环30次，约60s，等待网络注册成功
	uint8 count = 0;
    while (registerHomeNet != networkPara.REGstatus)
    {
        NetworkRegisterCheck(); // 获取网络注册情况
        count++;
        osDelay(2000);
        if(count>30)
        {
            printf("Modem: register timeout!");
            return;
        }
    }
    printf("Modem: register succeed!");
}


bool MqttConfig_Test(void)
{
    osDelay(5000);
    //start
    if(CMD_A7680CExecute(CMD_A7680C_CMMQTT_START, (void *)NULL) == false)
    {
        return false;
    }
    //accq
    MqttAccq_Info accq_test;
    char client_id[100] = {"DefaultProductKey.8212345678200235|securemode=3,signmethod=hmacsha1|"};
    accq_test.client_index = MQTT_CLIENT_INDEX;
    accq_test.clientID = client_id;
    accq_test.server_type = SERVER_TCP;
    if(CMD_A7680CExecute(CMD_A7680C_CMQTT_ACCQ, (void *)&accq_test) == false)
    {
        return false;
    }
    //connect
    ...
    subinfo.reqLength = 37;
    subinfo.qos = 1;
    if(CMD_A7680CExecute(CMD_A7680C_CMQTT_SUBSCRIBE, (void *)&subinfo) == false)
    {
        return false;
    }
	//$PERSISTENCE/upload/8212345678100189
    char topc_info[100] = {"$PERSISTENCE/adc/123"};
    if(CMD_A7680CExecute(CMD_A7680C_CMQTT_SUB_SEND, &topc_info) == false)
    {
        return false;
    }
	*/
    ...
    //topic
	char topc_info[100] = {"$PERSISTENCE/test/123456"};
    Topic_Info topicinfo;
    topicinfo.client_index = MQTT_CLIENT_INDEX;
    topicinfo.req_length = strlen(topc_info);
    if(CMD_A7680CExecute(CMD_A7680C_CMQTT_TOPIC, (void *)&topicinfo) == false)
    {
        return false;
    }
    if(CMD_A7680CExecute(CMD_A7680C_CMQTT_TOPIC_SEND, &topc_info) == false)
    {
        return false;
    }
    //payload
    ...
    char publish_string[] = {"puublish string test\r\n"};
    paylaod_info.req_length = strlen(publish_string);
    if(CMD_A7680CExecute(CMD_A7680C_CMQTT_PAYLOAD, (void *)&paylaod_info) == false)
    {
        return false;
    }
    if(CMD_A7680CExecute(CMD_A7680C_CMQTT_PAYLOAD_SEND, (void *)&publish_string) == false)
    {
        return false;
    }
    //pubilsh
	...
	if(CMD_A7680CExecute(CMD_A7680C_CMQTT_PUBISH, &pub_info) == false)
    {
        return false;
    }
	return true;
...
static int MODULE_Test_Demo(void)
{
    //Mqtt start
    memset(recv_buffer,0,100);
    memcpy(recv_buffer,"+CMQTTSTART: 0",100);
    cb_test.recLen = strlen(recv_buffer);
    bool ret = cmd_AnalyzeMqttStart(&cb_test,(void*)NULL);
    ret ? cout << "true" << endl :cout << "false" << endl;

    //AT+CMQTTACCQ=0,"DefaultProductKey.8212345678200235|securemode=3,signmethod=hmacsha1|",0
    ACCQ_Info accq_test;
    char client_id[100] = {"\"DefaultProductKey.8212345678200235|securemode=3,signmethod=hmacsha1|\""};
    accq_test.client_index = 0;
    accq_test.clientID = client_id;
    accq_test.server_type = MQTT_TCP;
    char mqtt_command[100];
    memset(mqtt_command, 0, 100);
    cb_test.cmd = mqtt_command;
    cmd_PackMqttAccq(&cb_test, (void*)&accq_test);
    printf("Pack : %s\n",cb_test.cmd);
